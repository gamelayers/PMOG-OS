...
namespace :s3 do
  ...
  namespace :backup do
    ...
    task :code  do
      ...
      make_bucket('code')
      archive = "/data/pmog/tmp/#{archive_name('code')}"
    ...
    task :db  do
      ...
      make_bucket('db')
      archive = "/data/pmog/tmp/#{archive_name('db')}"

      msg "retrieving db info"
      database, user, password, host = retrieve_db_info

      # the mysqldump is tuned to our large database sizes - duncan 11/02/09
      msg "dumping db"
      cmd = "mysqldump --single-transaction -u#{user} -h#{host}"
      ...
      # Hack to keep a local copy of the backup too - duncan 14/05/08
      msg "keeping a local gzipped copy"
      cmd = "tar zcvf /data/pmog/backups/pmog.`date +%Y-%m-%d-%H.%M`.sql.tar.gz #{archive}"
      system(cmd)
    ...
    task :scm  do
      ...
      make_bucket('scm')
      archive = "/data/pmog/tmp/#{archive_name('scm')}"
  ...
  def send_to_s3(name, tmp_file)
    archive = "/data/pmog/tmp/#{archive_name(name)}.tar.gz"
    ...
    begin
      ...
      retries_left -= 1
      msg "Caught #{e.class}: #{e.message}; #{retries_left} retries remaining"
  ...
  def retrieve_db_info
    ...
    # LOOK!
    # Note the logic for the host here. We want to dump the database from the mysql replicated slice,
    # rather than the main mysql slice, as it can impact performance. However, I don't want to hard code
    # all of the variables in case we switch servers at some point. If that happens, the script should
    # still run and we can update the host info for the new replicated host as and when. I'll most likely
    # forget to do this so at least we'll still have backups and a route to fix them once I notice
    # the production database is running dog slow... - duncan 14th May 2008
    return [
      config_file[RAILS_ENV]['database'],
      config_file[RAILS_ENV]['username'],
      config_file[RAILS_ENV]['password'],
      ( config_file[RAILS_ENV]['host'] == 'ey03-s00316' ? 'ey03-s00317' : config_file[RAILS_ENV]['host'] )
    ]
  end
