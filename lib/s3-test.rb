...
BUCKET_NAME = ("%s-test-bucket" % AWS_ACCESS_KEY_ID).downcase
CALLING_FORMATS = [
  [S3::CallingFormat::PATH, S3::BucketLocation::DEFAULT], 
  [S3::CallingFormat::SUBDOMAIN, S3::BucketLocation::DEFAULT], 
#  [S3::CallingFormat::SUBDOMAIN, S3::BucketLocation::EU]
]
...
class TC_AWSAuthConnectionTest < Test::Unit::TestCase
  def setup
    ...
    # clean up anything left over (assumes list and delete work..)
    begin
      response = @conn.list_bucket(BUCKET_NAME)
      response.entries.each do |entry|
        print "Deleting #{BUCKET_NAME}/#{entry.key}\n"
        @conn.delete(BUCKET_NAME, entry.key, {})
      end
  ...
  def test_operations
    # test all operations for both path-style and vanity domains
    ...
    CALLING_FORMATS.each do |settings|
      @conn.calling_format = settings[0]
      location = settings[1]
      ...
      reported_location = @conn.get_bucket_location(BUCKET_NAME).location
      if location == S3::BucketLocation::DEFAULT
        assert_equal("", reported_location, 'location matches')
      else
        assert_equal(location, reported_location, 'location matches')
      end
...
class TC_QueryStringAuthGeneratorTest < Test::Unit::TestCase
  ...
  def test_operations
    ...
    CALLING_FORMATS.each do |settings|
      ...
      @http.use_ssl = @generator.is_secure
