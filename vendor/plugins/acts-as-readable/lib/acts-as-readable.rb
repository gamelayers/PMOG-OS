  ...
  module Acts
    module Readable
      ...
      module SingletonMethods
        ...
        def count_unread_by(user)
          count(:all) - count_read_by(user)
        end

        def count_read_by(user)
          count(:all, :conditions => ["readings.readable_id = #{table_name}.id AND readings.user_id=?", user.id], :include => :readings)
        end
      end
