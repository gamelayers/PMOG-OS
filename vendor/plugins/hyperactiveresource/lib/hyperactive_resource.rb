...
#  But future changes aren't kept in sync (like ActiveRecord.. mostly)
  def method_missing(name, *args)
    ...
    when *self.has_manys.keys
      #puts "Has many gettr method"
      return has_many_getter_method_missing(name, self.has_manys[name])
    when *self.has_many_ids
      return has_many_ids_getter_method_missing(name)
    when *self.has_ones.keys
      return has_one_getter_method_missing(name, self.has_ones[name])      
    end                                     
  ...
  def has_many_getter_method_missing_unused( name )
    association_ids = self.send("#{name.to_s.singularize.underscore}_ids")
    puts "SEND #{name.to_s.singularize.underscore}_ids [#{association_ids}]"
  ...
  def has_many_getter_method_missing( name, args=nil)
    return nil if self.new?

    results = name.to_s.singularize.camelize.constantize.send("find_by_#{self.class.name.underscore}_id", self.id, args)
    results = [] if results.nil?

    return call_setter( name, results)
  end
  
  def has_one_getter_method_missing( name, args=nil )
    self.new? ? nil : 
      call_setter( name, name.to_s.singularize.camelize.constantize.send("find_by_#{self.class.name.underscore}_id", self.id, args) )
  end
  ...
  def self.find_or_create_by(field, *args )
    #puts "FOUND OR CREATE_BY #{field} [#{args[0]}]"

    val = self.find_by(:first, field, *args)
    return val if !val.nil?

    return create(field => args[0] )
  end
    
  FINDER_REGEXP = /^find_(?:(all)_?)?by_([a-zA-Z0-9_]+)$/ 
  FINDER_CREATE_REGEXP = /^find_or_create_by_([a-zA-Z0-9_]+)$/ 
  ...
  def self.method_missing( symbol, *args )
    ...
    elsif symbol.to_s =~ FINDER_CREATE_REGEXP 
      field_name = symbol.to_s.scan(FINDER_CREATE_REGEXP).first.first # yep, two firsts
      find_or_create_by(field_name, *args )        
    else
      super
    end
  end    
