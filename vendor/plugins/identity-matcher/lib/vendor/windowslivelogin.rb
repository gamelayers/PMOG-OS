...
class WindowsLiveLogin
  ...
  def debug(error)
    ...
    RAILS_DEFAULT_LOGGER.debug "Windows Live ID Authentication SDK #{error}"
...
class WindowsLiveLogin
  ...
  def processConsentToken(token, context=nil)
    if token.nil? or token.empty?
      ...
      puts("Error: processConsentToken: Null token.")
      return
    end
    decodedtoken = token
    parsedtoken = parse(CGI.unescape(decodedtoken))
    debug(parsedtoken.inspect)
    unless parsedtoken
...
class WindowsLiveLogin
  ...
  def derive(secret, prefix)
    puts "secret: #{secret}, prefix: #{prefix}"
    begin
